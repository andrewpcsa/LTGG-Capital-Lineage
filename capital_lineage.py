...
def add_edge_performance(lineage: Dict[str, Any], returns: ReturnStore, as_of: Optional[pd.Timestamp] = None) -> pd.DataFrame:
    ...
    if edges.empty:
        return edges
    nodes = lineage["nodes"].set_index("node_id").to_dict("index")
    ...
    for _, edge in edges.iterrows():
        ...
        if source == "External / existing cash":
            perf_rows.append({
                "edge_id": edge["edge_id"], "relative_total": np.nan, "relative_selection": np.nan,
                "actual_return": np.nan, "counterfactual_return": np.nan, "performance_coverage": 0.0,
                "performance_end": pd.NaT, "performance_truncated": False,
            })
            continue
        ...
        if valid_weight <= EPS:
            result = {
                "edge_id": edge["edge_id"], "relative_total": np.nan, "relative_selection": np.nan,
                "actual_return": np.nan, "counterfactual_return": np.nan, "performance_coverage": 0.0,
                "performance_end": pd.NaT, "performance_truncated": truncated,
            }
        else:
            # Normalize to the valid cohort weight. Ratios are based on ending wealth, which compounds correctly.
