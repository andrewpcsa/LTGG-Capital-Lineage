...
BUILD_VERSION = "2026-08-17 v6 — live SharePoint source"

# This identifies the existing LTGG workbook in SharePoint/OneDrive.
# It is not a password or access token. Authentication is supplied separately
# through Streamlit Secrets.
SHAREPOINT_DRIVE_ID = "b!HIcb8j8vIkmwoj0eNv9MoqEOwO5-qiZDoAKF3RGd5LjD9TXWJMRsSo3aMdNoRaIT"
SHAREPOINT_ITEM_ID = "01ELHLXYLFM6J5OYF47BE3ZEZ55ALLCAC6"
GRAPH_CONTENT_URL = (
    f"https://graph.microsoft.com/v1.0/drives/{SHAREPOINT_DRIVE_ID}/"
    f"items/{SHAREPOINT_ITEM_ID}/content"
)


def _microsoft_secrets():
    try:
        cfg = st.secrets["microsoft"]
        return cfg["tenant_id"], cfg["client_id"], cfg["client_secret"]
    except Exception as exc:
        raise RuntimeError(
            "Microsoft Graph credentials are not configured. Add tenant_id, client_id, "
            "and client_secret under [microsoft] in Streamlit Secrets."
        ) from exc


@st.cache_data(ttl=300, show_spinner=False)
def fetch_workbook_from_sharepoint() -> bytes:
    """Download the latest workbook from SharePoint/OneDrive using Microsoft Graph."""
    tenant_id, client_id, client_secret = _microsoft_secrets()
    ...
    if not access_token:
        detail = token_result.get("error_description") or token_result.get("error") or "Unknown authentication error"
    ...
    if not response.content:
        raise RuntimeError("SharePoint returned an empty workbook.")
...
source_col, refresh_col = st.columns([4, 1])
with source_col:
    st.caption("Data source: live LTGG trade-history workbook in SharePoint / OneDrive")
with refresh_col:
    if st.button("Refresh data", use_container_width=True):
        fetch_workbook_from_sharepoint.clear()
        load_data.clear()
        build_cached.clear()
        st.rerun()

try:
    with st.spinner("Loading latest workbook from SharePoint..."):
        file_bytes = fetch_workbook_from_sharepoint()
except Exception as exc:
    st.error("Could not load the LTGG workbook from SharePoint.")
    st.code(str(exc))
    st.info(
        "The app no longer needs the Excel file in GitHub. It does need Microsoft Graph "
        "credentials in Streamlit Secrets and read permission to the workbook."
    )
    st.stop()
